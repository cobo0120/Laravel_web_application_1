...
newTr.appendChild(newTd1);
  ...
  // name属性をselectタグに追加
  select1.setAttribute("name", "consumables_equipment");
...
select1.appendChild(option1);
  ...
  input2.value ='';
  input2.name = 'unit_purchase_price';
  input2.pattern = '^[0-9]+$';
  ...
  // name属性をselectタグに追加
  select2.setAttribute("name", "account");
...
let input2 = document.querySelectorAll("input[name=purchase_quantity]");
...
let consumables_equipment = document.querySelectorAll("select[name=consumables_equipment]");
...
const calc_subtotal = ()=>{
  ...
  input2 = document.querySelectorAll("input[name=purchase_quantity]");
  for(let i = 0;input1.length>i;i++){
  subtotal_price += Number(input1[i].value) * Number(input2[i].value);
  }

subtotal.value = Math.round(subtotal_price);
calc_total_amount();
}; 
...
const calc_tax_amount =()=>{
  input1 = document.querySelectorAll("input[name=unit_purchase_price]");
  input2 = document.querySelectorAll("input[name=purchase_quantity]");
  consumables_equipment = document.querySelectorAll("select[name=consumables_equipment]");
let sum_tax_amount = 0;
for(let i = 0;input1.length>i;i++){

if(consumables_equipment[i].value==4)
{
    sum_tax_amount += Math.floor(Number((input1[i].value) * Number(input2[i].value))*tax8);
} else{
    sum_tax_amount += Math.floor(Number((input1[i].value) * Number(input2[i].value))*tax10);
}
}
tax_amount.value = sum_tax_amount
};
