...
//        return view('posts.create_applicant', ['users' => $users])->render();
//    }
// }
...
// storeメソッドを用いてモデルにパラメータを渡す
   public function store(Request $request){
    ...
    Mail::to('test@*****com')->send(new SendTestMail($post));
    // Mail::to($post['mail_address'])->send(new SendTestMail($post));

    return back()->with('messages','申請をしました');
   }
...
public function index_history(Request $request){
    ...
    $posts = $query->select
    ('id','application_day','department_id','purchase','delivery_day','tax_amount')->paginate(5);// idと購入先を検索画面をページネーションで表示されるページを調整
